...
const ConfirmModal = ({ isOpen, onClose, onConfirm, title, message, confirmText = 'Confirm', type = 'warning' }) => {
  ...
  const buttonColors = {
    danger: 'bg-red-500 hover:bg-red-600 text-white transition-colors duration-150',
    warning: 'bg-amber-500 hover:bg-amber-600 text-white transition-colors duration-150',
    primary: 'btn-primary transition-colors duration-150'
  };

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="w-full max-w-md animate-in fade-in zoom-in-95 duration-200">
        <GlassCard className="relative overflow-hidden border border-zinc-800 shadow-2xl">
          {/* Modal Header */}
